...
// The Demo code is this file is designed to be easy to copy-and-paste in into
// your application! Because of this:
// - We never omit the ImGui:: namespace when calling functions, even though
// most of our code is already in the same namespace.
...
// unless it has been exposed in the public API (imgui.h).
// - We never use maths operators on ImVec2/ImVec4. For other imgui sources
// files, they are provided by imgui_internal.h w/ IMGUI_DEFINE_MATH_OPERATORS,
//   for your own sources file they are optional and require you either enable
//   those, either provide your own via IM_VEC2_CLASS_EXTRA in imconfig.h.
//   Because we don't want to assume anything about your support of maths
//   operators, we don't use them in imgui_demo.cpp.
...
#pragma clang diagnostic ignored \
    "-Wzero-as-null-pointer-constant"  // warning : zero as null pointer
                                       // constant                  // some
                                       // standard header variations use #define
                                       // NULL 0
#endif
...
// merged icon fonts (see misc/fonts/README.txt)
...
void ImGui::ShowDemoWindow(bool* p_open) {
  ...
  if (ImGui::CollapsingHeader("Help")) {
    ...
    ImGui::BulletText(
        "Please see the ShowDemoWindow() code in imgui_demo.cpp. <- you are "
        "here!");
  ...
  if (ImGui::CollapsingHeader("Configuration")) {
    ...
    if (ImGui::TreeNode("Configuration##2")) {
      ...
      ImGui::SameLine();
      HelpMarker(
          "Set to false to disable blinking cursor, for users who consider it "
          "distracting");
    ...
    if (ImGui::TreeNode("Backend Flags")) {
      HelpMarker(
          "Those flags are set by the back-ends (imgui_impl_xxx files) to "
          "specify their capabilities.");
      ImGuiBackendFlags backend_flags =
          io.BackendFlags;  // Make a local copy to avoid modifying actual
                            // back-end flags.
...
static void ShowDemoWindowWidgets() {
  ...
  if (ImGui::TreeNode("Basic")) {
    ...
    // framed elements (otherwise a Text+SameLine+Button sequence will have the
    // text a little too high by default)
...

    {
      ...
      HelpMarker(
          ...
          "InputText() to a dynamic string type. See misc/cpp/imgui_stdlib.h "
...

    {
      ...
      const char* element_names[Element_COUNT] = {"Fire", "Earth", "Air",
                                                  "Water"};
      static int current_element = Element_Fire;
      const char* current_element_name =
          (current_element >= 0 && current_element < Element_COUNT)
  ...
  if (ImGui::TreeNode("Trees")) {
    ...
    if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
      HelpMarker(
          ...
          "to select, CTRL+Click to toggle, click on arrows or double-click to "
          "open.");
  ...
  if (ImGui::TreeNode("Text")) {
    ...
    if (ImGui::TreeNode("Word Wrapping")) {
      ...
      ImGui::TextWrapped(
          ...
          "current implementation for text wrapping follows simple rules "
      ...
      ImGui::Text(
          ...
          "%.0f pixels. Testing a 1 character word. The quick brown fox jumps "
          "over the lazy dog.",
          wrap_width);
    ...
    if (ImGui::TreeNode("UTF-8 Text")) {
      ...
      // (e.g. Visual Studio save your file as 'UTF-8 without signature')
      // - FOR THIS DEMO FILE ONLY, BECAUSE WE WANT TO SUPPORT OLD COMPILERS, WE
      // ARE *NOT* INCLUDING RAW UTF-8 CHARACTERS IN THIS SOURCE FILE.
      ...
      ImGui::TextWrapped(
          ...
          "io.Font->AddFontFromFileTTF() manually to load extra character "
  ...
  if (ImGui::TreeNode("Images")) {
    ...
    // Here we are grabbing the font texture because that's the only one we have
    // access to inside the demo code. Remember that ImTextureID is just storage
    // for whatever you want it to be, it is essentially a value that will be
    // passed to the render function inside the ImDrawCmd structure. If you use
    // one of the default imgui_impl_XXXX.cpp renderer, they all have comments
    // at the top of their file to specify what they expect to be stored in
    // ImTextureID. (for example, the imgui_impl_dx11.cpp renderer expect a
    // 'ID3D11ShaderResourceView*' pointer. The imgui_impl_opengl3.cpp renderer
    // expect a GLuint OpenGL texture identifier etc.) If you decided that
    // ImTextureID = MyEngineTexture*, then you can pass your MyEngineTexture*
    // pointers to ImGui::Image(), and gather width/height through your own
    // functions, etc. Using ShowMetricsWindow() as a "debugger" to inspect the
    // draw data that are being passed to your render will help you debug issues
    ...
    if (ImGui::IsItemHovered()) {
      ...
      ImGui::Image(my_tex_id, ImVec2(region_sz * zoom, region_sz * zoom), uv0,
                   uv1, ImVec4(1.0f, 1.0f, 1.0f, 1.0f),
                   ImVec4(1.0f, 1.0f, 1.0f, 0.5f));
      ImGui::EndTooltip();
    }
  ...
  if (ImGui::TreeNode("Combo")) {
    ...
    // General BeginCombo() API, you have full control over your selection data
    // and display type. (your selection data could be an index, a pointer to
    // the object, an id for the object, a flag stored in the object itself,
    // etc.)
    const char* items[] = {"AAAA", "BBBB",    "CCCC", "DDDD",   "EEEE",
                           "FFFF", "GGGG",    "HHHH", "IIII",   "JJJJ",
                           "KKKK", "LLLLLLL", "MMMM", "OOOOOOO"};
    ...
    ImGui::Combo("combo 4 (function)", &item_current_4, &FuncHolder::ItemGetter,
  ...
  if (ImGui::TreeNode("Selectables")) {
    ...
    // application your selection may be stored in a different manner (in flags
    ...
    if (ImGui::TreeNode("Rendering more text into the same line")) {
      ...
      // and toggle your booleans automatically.
    ...
    if (ImGui::TreeNode("Grid")) {
      static bool selected[4 * 4] = {true,  false, false, false, false, true,
                                     false, false, false, false, true,  false,
                                     false, false, false, true};
      for (int i = 0; i < 4 * 4; i++) {
        ImGui::PushID(i);
        if (ImGui::Selectable("Sailor", &selected[i], 0, ImVec2(50, 50))) {
          // Note: We _unnecessarily_ test for both x/y and i here only to
          // silence some static analyzer. The second part of each test is
          // unnecessary.
          int x = i % 4;
          int y = i / 4;
          if (x > 0) {
    ...
    if (ImGui::TreeNode("Alignment")) {
      HelpMarker(
          ...
          "content.\nBy default, Selectables uses style.SelectableTextAlign "
          "but it can be overriden on a per-item basis using PushStyleVar().");
  ...
  if (ImGui::TreeNode("Text Input")) {
    ...
    if (ImGui::TreeNode("Resize Callback")) {
      // If you have a custom string type you would typically create a
      // ImGui::InputText() wrapper than takes your type as input. See
      // misc/cpp/imgui_stdlib.h and .cpp for an implementation of this using
      // std::string.
      HelpMarker(
          "Demonstrate using ImGuiInputTextFlags_CallbackResize to wire your "
          "resizable string type to InputText().\n\nSee "
          "misc/cpp/imgui_stdlib.h for an implementation of this for "
          "std::string.");
      struct Funcs {
        ...
        // Tip: Because ImGui:: is a namespace you would typicall add your own
        // function into the namespace in your own source files. For example,
        // you may add a function called ImGui::InputText(const char* label,
        // MyString* my_str).
  ...
  // Consider writing your own plotting widget, or using a third-party one (see
  // "Wiki->Useful Widgets", or github.com/ocornut/imgui/issues/2747)
  ...
  if (ImGui::TreeNode("Color/Picker Widgets")) {
    ...
    if (ImGui::BeginPopup("mypicker")) {
      ...
      for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++) {
        ...
        // (Note that ColorButton is already a drag source by default, unless
    ...
    HelpMarker(
        ...
        "display mode, but the user can change it with a "
        "right-click.\n\nColorPicker defaults to displaying RGB+HSV+Hex if you "
    ...
    HelpMarker(
        ...
        "ImGuiColorEditFlags_InputHSV allows you to store colors as HSV and "
        "pass them to ColorEdit and ColorPicker as HSV. This comes with the "
    ...
    ImGui::ColorEdit4("HSV shown as HSV##1", (float*)&color_stored_as_hsv,
                      ImGuiColorEditFlags_DisplayHSV |
                          ImGuiColorEditFlags_InputHSV |
                          ImGuiColorEditFlags_Float);
    ImGui::DragFloat4("Raw HSV values", (float*)&color_stored_as_hsv, 0.01f,
                      0.0f, 1.0f);
...
// natural type maximum, hence the /2 below.
    ...
    HelpMarker(
        ...
        "there is a user interaction.\nYou can override the clamping limits by "
        "using CTRL+Click to input a value.");
    ...
    ImGui::SameLine();
    HelpMarker(
        "You can use the 'power' parameter to increase tweaking precision on "
  ...
  if (ImGui::TreeNode("Drag and Drop")) {
    if (ImGui::TreeNode("Drag and drop in standard widgets")) {
      ...
      // in their drag and drop interaction. Also see the demo in Color Picker
      // -> Palette demo.
    ...
    if (ImGui::TreeNode("Drag to reorder items (simple)")) {
      // Simple reordering
      HelpMarker(
          "We don't use the drag and drop api at all here! Instead we query "
          "when the item is held but not hovered, and order items "
          "accordingly.");
  ...
  if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
    // Submit an item (various types available) so we can query their status in
    // the following block.
    ...
    ImGui::Combo(
        ...
        "repeat)"
        "\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0ColorEdit"
        "4\0MenuItem\0TreeNode\0TreeNode (w/ double-click)\0ListBox\0",
        20);
    ImGui::SameLine();
    HelpMarker(
...
static void ShowDemoWindowLayout() {
  ...
  if (ImGui::TreeNode("Child windows")) {
    ...
    // Child 1: no border, enable horizontal scrollbar
    {
      ...
      ImGui::BeginChild(
          "Child1", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f, 260),
          false, window_flags);
      for (int i = 0; i < 100; i++) {
    ...
    // styles)
    // - Using SetCursorPos() to position the child window (because the child
    // window is an item from the POV of the parent window)
  ...
  if (ImGui::TreeNode("Widgets Width")) {
    ...
    // Demonstrate using PushItemWidth to surround three items. Calling
    // SetNextItemWidth() before each of them would have the same effect.
  ...
  if (ImGui::TreeNode("Groups")) {
    HelpMarker(
        ...
        "functions such as IsItemHovered()/IsItemActive() or SameLine() etc. "
        "on the whole group.");
    ImGui::BeginGroup();
    {
      ImGui::BeginGroup();
  ...
  if (ImGui::TreeNode("Text Baseline Alignment")) {
    {
      ...
      HelpMarker(
          ...
          "keep it aligned with widgets. Lines only composed of text or "
      ...
      // text to upcoming widgets. Because we don't know what's coming after the
      // Text() statement, we need to move the text baseline down by
      // FramePadding.y
      ImGui::AlignTextToFramePadding();
  ...
  if (ImGui::TreeNode("Scrolling")) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (window_visible)  // Avoid calling SetScrollHereY when running with
                           // culled items
      {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      bool window_visible =
          ImGui::BeginChild(ImGui::GetID((void*)(intptr_t)i),
                            ImVec2(-100, child_height), true, child_flags);
    ...
    HelpMarker(
        ...
        "explicitly specify content width by calling "
    ...
    for (int line = 0; line < lines; line++) {
      ...
      // real application you may be better off manipulating the cursor position
      // yourself, aka using SetCursorPos/SetCursorScreenPos to position the
      // widgets yourself. You may also want to use the lower-level ImDrawList
      // API)
  ...
  if (ImGui::TreeNode("Clipping")) {
    ...
    ImGui::TextWrapped(
        ...
        "passing a scissor rectangle to the renderer. The system is designed "
        "to try minimizing both execution and CPU/GPU rendering cost.");
...
static void ShowDemoWindowPopups() {
  ...
  // of being held by the programmer as we are used to with regular Begin()
  // calls.
  ...
  if (ImGui::TreeNode("Popups")) {
    ImGui::TextWrapped(
        ...
        "behind the popup. Clicking outside the popup closes it.");
    ...
    // (If you want to show the current selection inside the Button itself, you
    // may want to build a string using the "###" operator to preserve a
  ...
  if (ImGui::TreeNode("Context menus")) {
    ...
    // For more advanced uses you may want to replicate and cuztomize this code.
    // This the comments inside BeginPopupContextItem() implementation.
    ...
    // providing an ID to BeginPopupContextItem(). BeginPopupContextItem() will
    // use the last item ID as the popup ID. In addition here, we want to
    // include your editable label inside the button label. We use the ###
...
static void ShowDemoWindowColumns() {
  ...
  */
  ...
  if (ImGui::TreeNode("Horizontal Scrolling")) {
    ...
    ImGui::BeginChild("##ScrollingRegion", ImVec2(0, ImGui::GetFontSize() * 20),
  ...
  if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
    ...
    if (ImGui::TreeNode("Dragging")) {
      ...
      ImGui::Text(
          ...
          "zero threshold: (%.1f, %.1f)\nMouseDelta: (%.1f, %.1f)",
          value_with_lock_threshold.x, value_with_lock_threshold.y, value_raw.x,
          value_raw.y, mouse_delta.x, mouse_delta.y);
      ImGui::TreePop();
    }
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
  ...
  HelpMarker(
      ...
      "are not affected. Use \"Export\" below to save them somewhere.");
  ...
  if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
    ...
    if (ImGui::BeginTabItem("Colors")) {
      ...
      for (int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
          ...
          // use icons. Read the FAQ and misc/fonts/README.txt about using icon
    ...
    if (ImGui::BeginTabItem("Fonts")) {
      ...
      HelpMarker(
          ...
          "_correct_ way of scaling your UI is currently to reload your font "
          "at the designed size, rebuild the font atlas, and call "
          "style.ScaleAllSizes() on a reference ImGuiStyle structure.");
      static float window_scale = 1.0f;
      if (ImGui::DragFloat("window scale", &window_scale, 0.005f, 0.3f, 2.0f,
                            ...
                            // constraints
  {
    static void Square(ImGuiSizeCallbackData* data) {
      data->DesiredSize.x = data->DesiredSize.y =
          (data->DesiredSize.x > data->DesiredSize.y ? data->DesiredSize.x
                                                     : data->DesiredSize.y);
    }
    ...
    if (ImGui::BeginTabItem("Primitives")) {
      ...
      for (int n = 0; n < 2; n++) {
        ...
        draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 0.0f,
                           corners_none, th);
        x += sz + spacing;  // Square
        draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col, 10.0f,
                           corners_all, th);
    ...
    if (ImGui::BeginTabItem("BG/FG draw lists")) {
      ...
      if (draw_bg)
        ImGui::GetBackgroundDrawList()->AddCircle(
            window_center, window_size.x * 0.6f, IM_COL32(255, 0, 0, 200), 48,
            10 + 4);
      if (draw_fg)
        ImGui::GetForegroundDrawList()->AddCircle(
            window_center, window_size.y * 0.6f, IM_COL32(0, 255, 0, 200), 48,
            10);
      ImGui::EndTabItem();
    }
