...
void OpenGLWindow::initializeGL() {
  ...
  // Load model
  loadModelFromFile(getAssetsPath() + "Lowpoly_tree_sample.obj");
...
void OpenGLWindow::paintGL() {
  ...
  for(int i = 0; i<19; i++){
    ...
    // Draw tree
    glm::mat4 model{1.0f};
    model = glm::translate(model, glm::vec3(eixo_x[i], 0.0f, eixo_y[i]));
  ...
  glUniformMatrix4fv(modelMatrixLoc, 1, GL_FALSE, &model[0][0]);
  glUniform4f(colorLoc, cor1[0], cor2[0], cor3[0], 1.0f);
  glDrawElements(GL_TRIANGLES, m_indices.size(), GL_UNSIGNED_INT, nullptr);
