...
void OpenGLWindow::initializeGL() {
  ...
  //iniciando o tabuleiro com numeros aleatórios
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
  ...
  ImGui::Begin("Jogo Maior Menor", nullptr, flags);
  {
    ...
    if (ImGui::BeginMenuBar()) {
      ...
      if (ImGui::BeginMenu("Game")) {
        ImGui::MenuItem("Restart", nullptr, &reset);
        ImGui::EndMenu();
      }
      // View menu
      /*
    ...
    //se o item no menubar for acionado para restart
    if(reset == true){ 
      Restart();
    }

    //gerando o tempo que é exibido na tela
    ...
    if (tempoJogo > 0) {
      ...
      //mostrando o que o jogador deve fazer
