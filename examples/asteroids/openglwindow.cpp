...
void OpenGLWindow::handleEvent(SDL_Event &event) {
  ...
  if (event.type == SDL_KEYDOWN) {
    if (event.key.keysym.sym == SDLK_SPACE)
      m_gameData.m_input.set(static_cast<size_t>(Input::Fire)); // Converte Fire para size_t, que é semelhante a um inteiro sem bit de sinal
...
void OpenGLWindow::update() {
  float deltaTime{static_cast<float>(getDeltaTime())}; // getDelta Time é importante
...
void OpenGLWindow::paintUI() {
...

  {
    ...
    ImGui::SetNextWindowPos(position); // Posiciona no centro da tela
