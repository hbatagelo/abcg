...
void Pads::initializeGL(GLuint program, int quantity) {
  terminateGL();
...
void Pads::update(const Ship &ship, float deltaTime) {    // no caso dos pads, nao precisamos receber nada, só a bolinha vai ter que receber as pads!
  for (auto &asteroid : m_asteroids) {
...
Pads::Pad Asteroids::createPad(glm::vec2 translation,
                                              float scale) {
  Pad asteroid;
  ...
  asteroid.m_angularVelocity = 0.0f;
