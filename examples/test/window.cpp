...
void Window::createBall() {
  ...
  // Generate VBO
  abcg::glGenBuffers(1, &m_ballVBO);
  abcg::glBindBuffer(GL_ARRAY_BUFFER, m_ballVBO);
  ...
  // Generate EBO
  abcg::glGenBuffers(1, &m_ballEBO);
  abcg::glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ballEBO);
  ...
  // Create VAO
  abcg::glGenVertexArrays(1, &m_ballVAO);
  ...
  abcg::glBindBuffer(GL_ARRAY_BUFFER, m_ballVBO);
  auto const positionAttribute{
      abcg::glGetAttribLocation(m_ballProgram, "inPosition")};
  abcg::glEnableVertexAttribArray(positionAttribute);
